...
- Blazing fast text searching and library management for Seforim.
...
** Exec Path From Shell (Nix provided)
...
* Spell Checking (Jinx — Nix provided)
...
** LaTeX Preamble Insertion Functions
...
#+begin_src emacs-lisp
  (defun my/create-hebrew-document (type)
    "Create Hebrew document of TYPE (latex/org/context/typst)."
    (interactive
     (list (completing-read "Document type: "
                            '("latex" "org" "context" "typst"))))
    (let ((filename (read-file-name (format "New Hebrew %s file: " type))))
      (find-file filename)
      (pcase type
        ("latex" (my/--insert-hebrew-latex-template))
        ("org" (my/--insert-hebrew-org-template))
        ("context" (my/--insert-hebrew-context-template))
        ("typst" (my/--insert-hebrew-typst-template)))
      (message "Hebrew %s document created." type)))

  (defun my/--insert-hebrew-latex-template ()
    "Insert Hebrew LaTeX template."
...
")
  ...
  (defun my/--insert-hebrew-org-template ()
    "Insert Hebrew Org template."
    (insert (format "#+TITLE: כותרת
#+AUTHOR: מחבר
#+DATE: %s
#+LATEX_CLASS: article-unlimited
#+LATEX_HEADER: \\babelprovide[main, import]{hebrew}
...
" (format-time-string "%Y-%m-%d")))
  ...
  (defun my/--insert-hebrew-context-template ()
    "Insert Hebrew ConTeXt template."
    (insert "\\mainlanguage[he]
\\setupalign[r2l]
...
")
  ...
  (defun my/--insert-hebrew-typst-template ()
    "Insert Hebrew Typst template."
    (insert "#set text(lang: \"he\", font: \"David CLM\")
...
")
  ...
  ;; Legacy keybindings for direct access
  (global-set-key (kbd "C-c h l") (lambda () (interactive) (my/create-hebrew-document "latex")))
  (global-set-key (kbd "C-c h o") (lambda () (interactive) (my/create-hebrew-document "org")))
  (global-set-key (kbd "C-c h x") (lambda () (interactive) (my/create-hebrew-document "context")))
  (global-set-key (kbd "C-c h t") (lambda () (interactive) (my/create-hebrew-document "typst")))
#+end_src
...
Complete indexed search system for a very large Hebrew text library.

| Tool        | Purpose              | Speed   |
|-------------|----------------------|---------|
| plocate     | Find by filename     | instant |
| recoll      | Full-text search     | instant |
| ripgrep-all | Fallback search      | fast    |
...
| F9              | my/toggle-hebrew-input              | Toggle Hebrew/English input        |
...
|-----------------+-------------------------------------+------------------------------------|
| C-c h n         | my/create-hebrew-document           | Create Hebrew document (any type)  |
| C-c h l         | (lambda)                            | Create Hebrew LaTeX doc            |
| C-c h o         | (lambda)                            | Create Hebrew Org doc              |
| C-c h x         | (lambda)                            | Create Hebrew ConTeXt doc          |
| C-c h t         | (lambda)                            | Create Hebrew Typst doc            |
...
|-----------------+-------------------------------------+------------------------------------|
| C-c S           | seforim-hydra/body                  | Launch Seforim Library UI          |
|-----------------+-------------------------------------+------------------------------------|
